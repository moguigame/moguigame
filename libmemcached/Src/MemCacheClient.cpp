    ...
    )
{
    ...
    try {
        ...
        if (rc != 0) {
            if (rc != SOCKET_ERROR || GetLastSocketError() != WSAEWOULDBLOCK) throw rc;
...
{
    strcpy_s(mAddress, rhs.mAddress);
    mIp   = rhs.mIp;
    mPort = rhs.mPort;
    mLastConnect = 0;
    return *this;
    ...
    ) 
{
    ...
    struct in_addr addr;
    addr.s_addr = mIp;
    _snprintf_s(mAddress, ADDRLEN, "%s:%d", inet_ntoa(addr), mPort);
    ...
    while (nItem < nItemCount) {
        for (nNext = nItem; nNext < nItemCount; ++nNext) {
            ...
            // create del request for all keys on this server
                ...
                sRequest += rgpItem[nNext]->mKey;
                sRequest += ' ';
                _snprintf_s(szBuf, sizeof(szBuf), "%ld", (long) rgpItem[nNext]->mExpiry);
                sRequest += szBuf;
    ...
    )
{
    ...
    sRequest += ' ';
    sRequest += a_pszKey;
    _snprintf_s(szBuf, sizeof(szBuf), " " SPRINTF_UINT64, a_nDiff);
    sRequest += szBuf;
    if (!a_bWantReply) {
        sRequest += " noreply";
    }
    ...
    )
{
    ...
    for (int n = 0; n < a_nCount; ++n) {
        ...
        _snprintf_s(szBuf, sizeof(szBuf), " %u %ld %u", 
            a_rgItem[n].mFlags, (long) a_rgItem[n].mExpiry, 
            a_rgItem[n].mData.GetReadSize());
        sRequest += szBuf;
        if (*a_pszType == 'c') { // cas
            _snprintf_s(szBuf, sizeof(szBuf), " " SPRINTF_UINT64, a_rgItem[n].mCas);
            sRequest += szBuf;
        }
...
{
    char szRequest[50];
    _snprintf_s(szRequest, sizeof(szRequest), 
        "flush_all %u\r\n", a_nExpiry);
